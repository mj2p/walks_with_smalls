...
// TODO: allow deleting individual features (#8972)
{
    ...
    function MapWidget(options) {
        ...
        // disable everything if not modifiable
        if (!this.options.modifiable) {
            this.disableModify();
        } else {
            // remove last point when esc | delete | backspace is pressed
            this.interactions.draw.on('drawstart',
                function(evt){
                    $(document).on('keyup', function(event){
                        if(event.keyCode === 27 || event.keyCode === 46 || event.keyCode === 8){
                            self.interactions.draw.removeLastPoint();
                        }
                    });
                }
            );
    ...
    MapWidget.prototype.generateStyles = function() {
    ...
    MapWidget.prototype.disableModify = function() {
    ...
    MapWidget.prototype.clearLastFeature = function() {
        if (this.interactions.draw) {
            this.interactions.draw.removeLastPoint();
        }
        this.enableDrawing();
    };
